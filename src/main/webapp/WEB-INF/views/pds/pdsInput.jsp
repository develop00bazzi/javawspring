...
<head>
	...
	<script>
		...
		function fCheck() {
			...
			if(file.trim()=="" || file==null) {
				alert("업로드할 파일명을 선택해주세요.");
				return false;
			}
			else if(title.trim()=="") {
				alert("업로드할 글 제목을 입력해주세요.");
			...
			for(let i=0; i<files.length; i++) {
				...
				if(file.name!="") {
					...
					alert("빈 파일 박스에 파일을 선택해주세요.");
					return false;
				}
			}
			
			if(fileSize>maxSize) {
				alert("업로드할 파일의 최대 용량은 20Byte입니다.");
				return false;
			}
			else {
...
<div class="container">
	<form name="myform" method="post" enctype="multipart/form-data">
		<h2 class="text-center">자 료  업 로 드</h2>
		<br/>
		...
		<div class="mb-2">
			<input type="button" value="자료 업로드" onclick="fCheck()" class="btn btn-secondary" /> &nbsp;
			<input type="reset" value="재작성" class="btn btn-secondary" /> &nbsp;
			<input type="button" value="돌아가기" onclick="location.href='${ctp}/pds/pdsList?part=${pageVo.part}&pag=${pag}&pageSize=${pageSize}';" class="btn btn-secondary" /> &nbsp;
		</div>
