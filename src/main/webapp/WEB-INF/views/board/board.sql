...
insert into board_s values (default,'관리자','게시판 서비스를 시작합니다.','dragon0hun@naver.com','https://github.com/develop00bazzi','이곳은 게시판입니다.',default,'192.168.50.65',default,default,'admin');

select * from board_s;

update boardReply_s set levelOrder=levelOrder+1 where boardIdx=42 and levelOrder>(select(select levelOrder from boardReply_s where idx=28) temp) and level=0;

/* 게시판에 댓글 달기 */
create table boardReply_s (
  ...
  level int not null default 0,						/* 댓글 레벨 - 첫번째 댓글 (부모 댓글)의 레벨은 0번 */
  levelOrder int not null default 0,				/* 댓글의 순서 - 첫번째 댓글 (부모 댓글)의 순서는 0번 */
  primary key(idx),
  foreign key(boardIdx) references board_s(idx)
  /* on update cascade */
  /* on delete restrict */
);
...
SELECT 	*,
  ...
  SELECT 	*,datediff(now(), wDate) as day_diff, timestampdiff(hour, wDate, now()) as hour_diff, (SELECT count(*)	FROM boardReply WHERE boardIdx=b.idx) as replyCount FROM board b order by idx desc limit ?,?";

  
  select levelOrder from boardReply_s where idx=27;
