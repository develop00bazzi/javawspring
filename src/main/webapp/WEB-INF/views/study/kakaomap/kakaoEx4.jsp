    ...
    <script>
    	...
    	var longitude='${vo.longitude}';
   		...
   		function addressSearch() {
    		...
    		location.href="${ctp}/study/kakaomap/kakaoEx4?address="+address;
    	}
    </script>
</head>
<body>
			...
			<div>
				...
				<select name="address" id="address" class="form-control mb-3">
					...
					<c:forEach var="vo" items="${vos}">
						<option <c:if test="${vo.address==address}">selected</c:if>>${vo.address}</option>
					</c:forEach>
				</select>
				<input type="button" value="지역 검색" onclick="addressSearch()" class="btn btn-secondary mb-3"/>
			</div>
		</form>
	</div>
